        ...
        for script in ["cd4.py", "maria.py", "nod.py"]:
            for model in best_models.values():
                if model["features"] == "seq_only":
                    evaluation_script_params = f"python ./src/mhciipresentation/evaluation/{script} --model_wo_pseudo_path {model['model']} --data {model['data']} --features {model['features']}"
                else:
                    evaluation_script_params = f"python ./src/mhciipresentation/evaluation/{script} --model_with_pseudo_path {model['model']} --data {model['data']} --features {model['features']}"
