...
def ds_stats(ds, ys):
    ...
    num_EL_alleles_human=len([el for el in set(ds[ys!=0][ds[ys!=0]["file_name"].str.contains("_EL")]["MHC_molecule"]) if not "mouse" in el] )
    num_EL_alleles_mouse=len([el for el in set(ds[ys!=0][ds[ys!=0]["file_name"].str.contains("_EL")]["MHC_molecule"]) if  "mouse" in el] )
    ...
    num_BA_exp_alleles_human = len([el for el in set(ds[ds["file_name"].str.contains("_BA")]["MHC_molecule"]) if not "H-2" in el] )
    num_BA_exp_alleles_mouse = len([el for el in set(ds[ds["file_name"].str.contains("_BA")]["MHC_molecule"]) if  "H-2" in el] )
