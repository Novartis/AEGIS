...
def build_dest_dir(ext):
    dest_dir = here() / f"scripts/figures/generated/ablations/{ext}"
...
def make_plot(feat, ds, emb, all_ones, seed):
    ...
    annot = f"Feature set: {feat}, Data source: {ds}, Emb: {emb}, All ones: {all_ones}"
    ext = f"{feat}-{ds}-{emb}-{all_ones}-{seed}"
